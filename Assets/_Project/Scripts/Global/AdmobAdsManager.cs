...
public class AdmobAdsManager : AbstractAdsmanager
{
    ...
    void Awake()
    {
        ...
        mediation = FindObjectOfType<AbstractAdsmanager>();
    ...
    void Start()
    {
        ...
        if (Application.internetReachability != NetworkReachability.NotReachable && (PlayerPrefs.GetInt("RemoveAds") != 1))
        {
    ...
    public void ShowRewardedCoin()
    {
        if (mediation != null && Application.internetReachability != NetworkReachability.NotReachable)
        {
            //  if (mediation.IsRewardedAdReady())
            //   {
            mediation.ShowRewardedVideo(GiveCoin);
            //  }
            //   else {
    ...
    {
        if (IsSmallBannerReady() || smallBannerStatus == AdLoadingStatus.Loading || adsStatus == AdType.NOAds || Noads())
        {
            return;
        }
    ...
    {
        if (IsSmallAdaptiveBannerReady() || smallAdaptiveBannerStatus == AdLoadingStatus.Loading || adsStatus == AdType.NOAds || Noads())
        {
            return;
        }
        if (Application.internetReachability == NetworkReachability.ReachableViaCarrierDataNetwork | Application.internetReachability == NetworkReachability.ReachableViaLocalAreaNetwork)
    ...
    {
        if (IsSmallSmartBannerReady() || smallSmartBannerStatus == AdLoadingStatus.Loading || adsStatus == AdType.NOAds || Noads())
        {
            return;
        }
        if (Application.internetReachability == NetworkReachability.ReachableViaCarrierDataNetwork | Application.internetReachability == NetworkReachability.ReachableViaLocalAreaNetwork)
    ...
    {
        if (IsMediumBannerReady() || mediumBannerStatus == AdLoadingStatus.Loading || adsStatus == AdType.NOAds || Noads())
        {
            return;
        }
        if (Application.internetReachability == NetworkReachability.ReachableViaCarrierDataNetwork | Application.internetReachability == NetworkReachability.ReachableViaLocalAreaNetwork)
    ...
    {
        if (!isAdmobInitialized || IsInterstitialAdReady() || iAdStatus == AdLoadingStatus.Loading || adsStatus == AdType.NOAds || Noads())
        {
            //print("isAdmobInitialized :"+ isAdmobInitialized + "IsInterstitialAdReady "+ IsInterstitialAdReady() + " iAdStatus :"+ AdLoadingStatus.Loading + "adsStatus :"+ AdType.NOAds);
            return;
        }
        if (Application.internetReachability == NetworkReachability.ReachableViaCarrierDataNetwork | Application.internetReachability == NetworkReachability.ReachableViaLocalAreaNetwork)
        {
    ...
    {
        if (!isAdmobInitialized || IsVideoAdReady() || vAdStatus == AdLoadingStatus.Loading || Constants.adsRemoteConfigStatus == "0" || adsStatus == AdType.NOAds || Noads())
        {
            return;
        }
        if (Application.internetReachability == NetworkReachability.ReachableViaCarrierDataNetwork | Application.internetReachability == NetworkReachability.ReachableViaLocalAreaNetwork)
    ...
    {
        if (!isAdmobInitialized || IsRewardedAdReady() || rAdStatus == AdLoadingStatus.Loading || Constants.adsRemoteConfigStatus == "0" || Noads())
        {
            return;
        }
    ...
    {
        if (!isAdmobInitialized || IsRewardedInterstitialAdReady() || riAdStatus == AdLoadingStatus.Loading || Constants.adsRemoteConfigStatus == "0" || Noads())
        {
    ...
    public override void ShowSmallBanner(AdPosition position)
    {
        ...
        try
        {
            if (!isAdmobInitialized || !isAdmobBannerInitialized || Noads())
            {
                return;
            }
            if (SmallBanner_High_Ecpm)
    ...
    public void MediumBanner_HandleOnAdLoaded3(object sender, EventArgs args)
    {
        MobileAdsEventExecutor.ExecuteInUpdate(() =>
        {
            if (MediumBanner_Low_Ecpm)
            {
                ...
                //callBackText.text = "Medium Banner Loaded of Size: " + BannersHandler.instance.width.text + "*" + BannersHandler.instance.height.text;
                AdmobGA_Helper.GA_Log(AdmobGAEvents.MediumBannerLoaded_L_Ecpm);
